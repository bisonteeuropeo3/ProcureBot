...
import { FileText, Loader2, Clock, AlertTriangle, RotateCcw } from 'lucide-react';
import { Receipt } from '../types';

interface PendingReceiptsListProps {
    receipts: Receipt[];
    onReview: (receipt: Receipt) => void;
    onRetryAnalysis: (receipt: Receipt) => void;
}

const PendingReceiptsList: React.FC<PendingReceiptsListProps> = ({ receipts, onReview, onRetryAnalysis }) => {
                ...
                {receipts.map((receipt) => (
                    <div key={receipt.id} className="p-4 flex items-center justify-between hover:bg-cream/50 transition-colors">
                        <div className="flex items-center gap-4">
                            <div className={`w-10 h-10 flex items-center justify-center border ${receipt.status === 'failed'
                            ...
                            <div>
                                ...
                                {/* Show error message for failed receipts */}
                                {receipt.status === 'failed' && (receipt as any).last_error && (
                                    <p className="text-xs text-red-500 mt-1 max-w-md truncate" title={(receipt as any).last_error}>
                                        ⚠️ {(receipt as any).last_error}
                                    </p>
                                )}
                            </div>
                        </div>
                        ...
                        <div className="flex items-center gap-4">
                            ...
                            {receipt.status === 'analyzed' ? (
                                ...
                                <button
                                    onClick={() => onRetryAnalysis(receipt)}
                                    className="px-4 py-2 bg-red-500 text-white border border-charcoal font-bold text-xs hover:bg-red-600 transition-colors shadow-[2px_2px_0px_#1A1E1C] flex items-center gap-1"
                                >
